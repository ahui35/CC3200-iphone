...
@interface BluRayViewController ()
{
  NSTimer *timer;
  UIButton *currentBtn;
}
...
@implementation BluRayViewController
...
- (void)viewDidLoad {
  ...
  sv.contentSize = CGSizeMake(Main_Screen_Width, 680);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    int column = i%3;
    btn = [[RemoteButton alloc] init];
    btn.frame = CGRectMake(12 + 80*column, 284 + 64 + 53 *row, 77, 49);
  ...
  btn = [[RemoteButton alloc] init];
  btn.frame = CGRectMake(250, 283 + 64, 62, 105);
  [btn setBackgroundImage:[UIImage imageNamed:@"bt_home"] forState:UIControlStateNormal];
  [sv addSubview:btn];
  btn.action = @"home";
  [btn addTarget:self action:@selector(responseToBtn:) forControlEvents:UIControlEventTouchUpInside];
  
  UIImageView *imageV = [[UIImageView alloc] initWithFrame:CGRectMake(11, 64 + 465, 174, 50)];
  ...
  for (int i = 0; i < 5; i++) {
    btn = [[RemoteButton alloc] init];
    btn.frame = CGRectMake(10 + 62*i, 64 + 397, 58, 58);
  ...
  for (int i = 0; i < 5; i++) {
    btn = [[RemoteButton alloc] init];
    btn.frame = CGRectMake(10 + 62*i, 64 + 463, 58, 58);
    if (i < 3) {
      btn.frame = CGRectMake(13 + 62*i, 64 + 467, 48, 48);
    }
    btn.remoteType = [[array objectAtIndex:i] intValue];
    [sv addSubview:btn];
